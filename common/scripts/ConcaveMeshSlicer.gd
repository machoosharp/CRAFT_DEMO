...
func _convertToArrayMesh( mesh: Mesh ):
	...
	for i in range( mesh.get_surface_count() ):
		...
		new_mesh.add_surface_from_arrays(
			...
			surface_tool.commit().surface_get_arrays( 0 )
		)
...
func slice_mesh(
	...
	# converts Mesh to ArrayMesh
	var mesh: ArrayMesh = _convertToArrayMesh( in_mesh )

	# Gets normal and origin of the slicer body
	var normal = -slice_transform.basis.z
	var at     =  slice_transform.origin

	# Creates two surface tools for building the two meshes we expect to create.
	...
	# Clears the vertex slice list
	# Still need to understand the use of this
	vert_slice.clear()

	# Grabs Material of existing mesh if not provided one
	...
	for surface_idx in range( mesh.get_surface_count() ):
		...
		# Mesh data tool is like a mesh analizer tool that is used to get 
		# extra infomation we will need to slice the mesh, such as
		# edges and faces. we apply one surface at a time to mdt in
		# order to work on mesh sections appropriately.
		...
		# Surface tools for each surface
		# https://docs.godotengine.org/en/stable/tutorials/3d/procedural_geometry/surfacetool.html
		var surface_tool := SurfaceTool.new()
		...
		# For each vertex, get the side of the slicer plane that its on and 
		# append that side value to the verts_side list
		...
		for edge in range( mdt.get_edge_count() ):
			...
			# Get indexes of vertexes on edge
			var v1 = mdt.get_edge_vertex( edge, 0 )
			var v2 = mdt.get_edge_vertex( edge, 1 )

			# Using the index of the vertex, get the sides and compare them
			# If sides don't match, the edge is crossed by the slicer,
			# therefor it would count as an intersection and be sliced.
			if verts_side[v1] != verts_side[v2] and verts_side[v2] != 0 and verts_side[v1] != 0:
				...
				# Should return the vertex of the intersection
				var intersect: Vector3 = _line_plane_intersection( v1_at, v2_at, at, normal )

				if intersect != null:
					if intersect != v2_at and intersect != v1_at:
						var norm = mdt.get_vertex_normal( v2 )
						var uv = mdt.get_vertex_uv( v1 ) + ( 
							mdt.get_vertex_uv( v2 ) - mdt.get_vertex_uv( v1 ) 
						) * (
							v1_at.distance_to( intersect ) / v1_at.distance_to( v2_at ) 
						)

						# builds a vertex at the intersection of edges
						edge_slice[ edge ] = [ intersect, norm, uv ]

		# this section of code above is the section we need to edit to prevent full
		# slices, there is nothing in this code that says the slicer shouldn't
		# slice if the slicer itself doesn't intersect, the only thing it grabs is
		# the plane of the slicer and slices everything in the sliced mesh along
		# that plane to inifinity. This should be changed to only slice the edges
		# that are crossed directly by the slicer.
		...
		for face in range( mdt.get_face_count() ):
			...
			# For each edge in a face, if the edge is in edge_slice then it is
			# intersecting the slicer, append to edge_instersect.
			for e in range( 3 ):
			...
			if len( edge_intersect ) == 0:
				...
				for v in range( 3 ):
					...
					if _check_side( vv1, normal, at ) == 0 and _check_side( vv2, normal, at ) == 0:
						_add_to_vert_slice(
							mdt.get_face_vertex( face, v ),
							mdt.get_face_vertex( face, (v + 1) % 3 )
						)
						break
	...
	for polygon in sorted_verts:
		...
		for v in range( float( len( triangulate ) ) / 3 ):
			...
			var abc = [vert1, vert2, vert3]
			var cba = [vert3, vert2, vert1]

			var vv_side = _check_side(
				_get_triangle_normal( vert1, vert2, vert3 ), 
				normal, 
				Vector3.ZERO 
			)
			...
			if vv_side != -1:
				for vv in abc:
					surface_tool1_2.set_uv( _get_uv( slice_transform, vv ) )
					surface_tool1_2.add_vertex( vv )

				for vv in cba:
					surface_tool2_2.set_uv( _get_uv( slice_transform, vv ) )
					surface_tool2_2.add_vertex( vv )
	...
	for i in range( 0, len( surfaces1 ) ):
		if surfaces1[i].get_surface_count() != 0:
			new_mesh.add_surface_from_arrays(
				Mesh.PRIMITIVE_TRIANGLES,
				surfaces1[i].surface_get_arrays( 0 )
			)
			new_mesh.surface_set_material(
				new_mesh.get_surface_count() - 1,
				surfaces_mat1[i]
			)
	...
	for i in range( 0, len( surfaces2 ) ):
		if surfaces2[i].get_surface_count() != 0:
			new_mesh2.add_surface_from_arrays( 
				Mesh.PRIMITIVE_TRIANGLES,
				surfaces2[i].surface_get_arrays( 0 )
			)
			new_mesh2.surface_set_material(
				new_mesh2.get_surface_count() - 1,
				surfaces_mat2[i]
			)
				...
				for j in range( len( polygons ) ):
					...
					if j != i:
						...
						for k in range( len( polygon_check ) ):
							var local_pos = _to_transform_local2(
								slice_transform,
								polygon_check[k]
							)
							var pos2d = Vector2( local_pos.x, local_pos.y )

							local_pos = _to_transform_local2(
								slice_transform,
								polygon_check[(k + 1) % len( polygon_check )]
							)
							var pos2d2 = Vector2( local_pos.x, local_pos.y )

							polygon2d.append( pos2d )

							var point_segment = Geometry2D.get_closest_point_to_segment(
								point,
								pos2d,
								pos2d2
							)
					...
					while not connected:
						...
						for j in range( len( polygon ) ):
							...
							for l in range( len( polygon_to ) ):
								...
								for k in range( len( polygons ) ):
									...
									for m in range( len( polygon_check ) ):
										...
										if v1 != vert_to and v1 != vert and v2 != vert_to and v2 != vert:
											var intersect = _line_plane_intersection(
												v1, 
												v2, 
												vert,
												n
								...
								if not is_intersect:
									var connection_norm = _get_triangle_normal(
										polygon[j],
										polygon_to[l],
										polygon_to[l] + norm
									)
									var e_norm = _get_triangle_normal(
										polygon_to[( l + 1 ) % len( polygon_to )],
										polygon_to[l],
										polygon_to[l] + norm
									)
									var e_norm2 = _get_triangle_normal(
										polygon_to[posmod( l - 1, len( polygon_to ) )],
										polygon_to[l],
										polygon_to[l] + norm
									)

									var xbt = _find_closest_edge(
										e_norm2,
										e_norm,
										connection_norm,
										polygon_to[posmod( l - 1, len( polygon_to ) )],
									...
									e_norm = _get_triangle_normal(
										polygon[j],
										polygon[posmod( (j - 1), len( polygon ) )],
										polygon[j] + norm
									)

									e_norm2 = _get_triangle_normal(
										polygon[j],
										polygon[posmod( (j + 1), len( polygon ) )],
										polygon[j] + norm
									)

									var closes_edge = _find_closest_edge(
										e_norm2,
										e_norm,
										connection_norm,
										polygon[posmod( (j + 1), len( polygon ) )],
									...
									if closes_edge == 0:
										for k in range( len( polygon ) ):
											polygons[nearest_intersect_polygon].insert(
												l + 1 + k,
												polygons[i][(k + j) % len( polygon )]
											)
										...
										for k in range( 0, len( polygon ) ):
											polygons[nearest_intersect_polygon].insert(
												posmod( 
													l + 1 + k, 
													len( polygons[nearest_intersect_polygon] )
												),
												polygons[i][posmod( (j - k), len( polygon ) )]
											)
									...
									polygons[nearest_intersect_polygon].insert(
										...
										polygons[nearest_intersect_polygon][ l % len( polygon_to )]
									)
...
func _check_side(
	point: Vector3,    # Vertex on a surface of object being sliced.
	normal: Vector3,   # Slicer normal
	plane_at: Vector3  # Slicer Origin
):
	# Relative position of vertex to plane, essentially put the origin of the
	# mesh and the plane on the same point, 
	var relative_pos = plane_at - roun( point )
	
	# Get the dot product of the point compared to the slicer normal plane,
	# basically just tells us which side of the plane are we on, 1 and -1 are
	# left or right, 0 is right on the plane.
	var dot = roun( normal.dot( relative_pos ) )
	
	# Returns the "positive/negative" of the dot product, basically 
	# just simplifies it down to 1, 0, -1.
	return -sign(dot)
...
func _line_plane_intersection(
	line_start: Vector3, # Vector of vertex on edge
	line_end: Vector3,   # Vector of vertex on edge
	plane_at: Vector3,   # slicer origin
	plane_norm: Vector3  # slicer normal 
):
	roun( line_start )
	roun( line_end )

	# line as if it were starting from origin
	var v = line_end - line_start

	# 
	var w = plane_at - line_start
	
	var w_riz = w.dot(plane_norm)
	var v_dot = v.dot(plane_norm)
	
	var k = w_riz/v_dot

	if k >= 0 and k <= 1:
		return roun( (line_start + k * v) )
	return null
